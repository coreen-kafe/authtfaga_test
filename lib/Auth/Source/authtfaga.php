...
class sspmod_authtfaga_Auth_Source_authtfaga extends SimpleSAML_Auth_Source
{
    ...
    const TFAAUTHNCONTEXTCLASSREFALT = 'https://refeds.org/profile/mfa';
    ...
    public $tfa_authencontextclassrefs;
	...
	$this->tfa_authencontextclassrefs = array( self::TFAAUTHNCONTEXTCLASSREF, self::TFAAUTHNCONTEXTCLASSREFALT );
    ...
    private function createTables()
    {
        ...
        $q = 'CREATE TABLE IF NOT EXISTS sspga_status (
		          ...
		          PRIMARY KEY(uid),
		          enable BOOL,
			  status VARCHAR(60) NOT NULL DEFAULT "initial"
		         );';
        $result = $this->dbh->query($q);
    }

    public function registerCCref($ref) {
	$this->tfa_authencontextclassref = $ref;
    }
    
    public function getCCref() {
	return $this->tfa_authencontextclassref;
    } 

    public function sendQRcode($email, $code, $admail) {
	$time = date(DATE_RFC822);
	$srvName = $_SERVER['SERVER_NAME'];
	if($isIP = (bool)ip2long($srvName)) $srvName = "hiddendomain.url";
	
	$message = '<h1>Your QR code for strong authentication</h1><p>Sent from <tt> '. $srvName .' </tt>at '. $time . '</p>' .
		'<p><ul><li>Please securely keep this email.</li><li> You can reconfigure your Google authenticator with this QR code if you lost the authenticator.</li></p>'.'<p><img src='.$code.'</p><h2>How to use</h2><p><ul><li>Read this QR code to get an authentication code (You only need to do this once)</li><li>If you did not issue this email, please contact your IT administration staff ('.$admail.') as soon as possible </li></ul></p>';
	$config = SimpleSAML_Configuration::getInstance();
	$fromaddress = $config->getString('technicalsupport', 'noreply@' . $srvName);
	$toaddress = $config->getString('technicalsupport', 'jiny92@kisti.re.kr');
	$email = new SimpleSAML_XHTML_EMail($toaddress, 'Strong authentication for KAFE members', $fromaddress);
	$email->setBody($message);
	$email->send();

    }

    public function enable2fa($uid, $opt)
    {
        $q = "REPLACE INTO sspga_status SET enable='$opt', uid='$uid'";
        $result = $this->dbh->query($q);
    ...
    public function update2fa($uid, $opt)
    {
        $q = "UPDATE sspga_status SET enable='$opt' WHERE uid ='$uid'";
        $result = $this->dbh->query($q);

        SimpleSAML_Logger::info('authtfaga: '.$uid.' updates the two-factor authentication.');

        return true; 

    }

    public function reset2fa($uid, $opt, $stat)
    {
	$q = "UPDATE sspga_status SET enable='$opt', status='$stat' WHERE uid='$uid'";
	$result = $this->dbh->query($q);
	if($opt == 0) {
	    $q = "DELETE from sspga_gakeys WHERE uid='$uid'";
	    $result = $this->dbh->query($q);
	}
	return true;
    }
    ...
    public function remove2fa($uid)
    {
	$q = "DELETE FROM sspga_status where uid='$uid'";
	$result = $this->dbh->query($q);
	$q = "DELETE FROM sspga_gakeys where uid='$uid'";
	$result = $this->dbh->query($q);
        SimpleSAML_Logger::info('authtfaga: '.$uid.' delete existing two-factor authentication.');
    }
    ...
    {
	$q = "SELECT status FROM sspga_status WHERE uid='$uid'";
	$result = $this->dbh->query($q);
	$row = $result->fetch();
	$status = $row[0];
	return $status;
    ...
    public function setStatus($uid, $stat)
    {
	$q = "UPDATE sspga_status SET status='$stat' WHERE uid ='$uid'";
	$result = $this->dbh->query($q);
